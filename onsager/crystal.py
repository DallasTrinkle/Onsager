...
class GroupOp(collections.namedtuple('GroupOp', 'rot trans cartrot indexmap')):
    ...
    def __sane__(self):
        ...
        tr = self.rot.trace()
        det = int(np.round(np.linalg.det(self.rot)))
        # consistency:
        if int(np.round(self.cartrot.trace())) != tr: return False
        if int(np.round(np.linalg.det(self.cartrot))) != det: return False
        # sanity:
